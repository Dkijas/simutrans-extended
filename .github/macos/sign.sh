...
# (see LICENSE.txt)
#
# Sign "Simutrans Extended.app" with the Developer ID identity held in the
...
#   * it does not pass --entitlements.  That was re-measured on Simutrans-
#     Extended rather than inherited from Standard.  At cc56217cf the tree has
#     no dlopen, dlsym or dlclose at all - zero occurrences, the vendored
#     squirrel/ included - no mprotect, PROT_EXEC or MAP_JIT anywhere, and no
#     AVCaptureDevice, AVAudioRecorder or CLLocationManager.  macOS audio is
#     AVAudioPlayer in sound/AVF_core-audio_sound.mm, which is playback only
#     and needs no microphone entitlement; outgoing and listening sockets need
#     none outside the App Sandbox.  Every library the bundle loads ships
#     inside it and is signed here with the same Team ID, so the Hardened
#     Runtime's library validation is satisfied without disabling it.  An empty
#     entitlement set is the minimum that works, and adding entitlements
#     "just in case" would weaken the runtime.
...
KEYCHAIN_PATH=${SIMU_KEYCHAIN_PATH:-${RUNNER_TEMP:-/tmp}/simutrans-extended-signing.keychain-db}
