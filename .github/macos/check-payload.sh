...
#
# Usage: check-payload.sh <extracted-root> <expected-app-relative-path> [strict|allow-data-siblings]
#
# The third argument defaults to `strict`, which is Simutrans Standard's
# behaviour unchanged: one bundle, and nothing whatever beside it.
#
# Simutrans-Extended cannot use that mode, and the reason is in the product,
# not in the packaging.  Standard installs its data inside the bundle at
# Contents/Resources/simutrans.  Extended's simmain.cc strips the
# ".app/Contents/MacOS/" suffix from argv[0] and then looks for config/,
# font/, text/ and the paksets in the directory that CONTAINS the application,
# so the data has to travel beside the .app or the game does not start.
#
# `allow-data-siblings` keeps every other check and replaces "nothing beside
# the bundle" with the rule that actually matters once data is expected there:
# no Mach-O file may exist outside the bundle.  Code that ships outside the
# bundle is code that is never signed, never stapled and never assessed.
#
...
#     one of them,
#   * nothing outside the bundle came along for the ride (strict), or that
#     nothing outside it is executable code (allow-data-siblings).

set -euo pipefail

ROOT=${1:?usage: check-payload.sh <extracted-root> <expected-app-relative-path> [strict|allow-data-siblings]}
EXPECTED_APP=${2:?expected .app path, relative to the extraction root}
SIBLING_MODE=${3:-strict}

case "$SIBLING_MODE" in
	strict|allow-data-siblings) ;;
	*)
		echo "::error::unknown sibling mode '$SIBLING_MODE' (use strict or allow-data-siblings)"
		exit 1
		;;
esac
...
if [ "$SIBLING_MODE" = "strict" ]; then
	# Standard's rule, unchanged: the build archives one directory that
	# contains one application, and anything else is a surprise worth
	# stopping on.
	if [ -n "$strays" ]; then
	...
	# Extended's rule.  Data beside the bundle is not a surprise here, it is
	# how the application finds config/, font/, text/ and the paksets at all,
	# so "nothing beside the bundle" cannot be the test.  What must still be
	# true is that no CODE sits outside the bundle: every Mach-O that ships
	# has to be inside the thing that gets signed, stapled and assessed by
	# Gatekeeper.  A Mach-O out here would be shipped unsigned and unchecked,
	# which is the failure the strict rule was really guarding against.
	echo "   sibling data is permitted in this mode; checking that none of it is code"
	if [ -n "$strays" ]; then
		printf '%s\n' "$strays" | sed "s|^$ROOT/|     |"
	else
		echo "     (nothing beside the bundle)"
	fi
	code_outside=0
	while IFS= read -r -d '' f; do
		case "$f" in
			"$ROOT/$EXPECTED_APP"/*) continue ;;
		esac
		if file -b "$f" | grep -q 'Mach-O'; then
			note_failure "Mach-O file outside the bundle: ${f#"$ROOT"/}"
			code_outside=$((code_outside + 1))
		fi
	done < <(find "$ROOT" -type f -print0)
	echo "   Mach-O files outside the bundle: $code_outside"
fi
...
echo "payload check passed: $entries entries, $symlinks symlink(s), one bundle at $EXPECTED_APP (siblings: $SIBLING_MODE)"
